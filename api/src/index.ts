  ...
  async fetch(
    ...
    if (path === "/api/comments" && request.method === "GET") {
      ...
      const commentsWithReplies = await Promise.all(comments.results.map(async (comment: any) => {
        ...
        // 为每个回复获取用户信息
        const repliesWithUsers = await Promise.all(replies.results.map(async (reply: any) => {
          const replyUser = await env.DB.prepare(
            "SELECT id, username, nickname, avatar, role FROM users WHERE id = ?"
          ).bind(reply.userId).first();
          reply.user = replyUser;
          return reply;
        }));
        
        comment.replies = repliesWithUsers;
        return comment;
      }));
    ...
    if (path.startsWith("/api/comments/") && path.endsWith("/like") && request.method === "POST") {
      const id = path.replace("/api/comments/", "").replace("/like", "");
      // 检查评论是否存在
      ...
      // 增加点赞数
      const updatedLikes = (Number(comment.likes) || 0) + 1;
      await env.DB.prepare(
        "UPDATE comments SET likes = ?, updateTime = ? WHERE id = ?"
      ).bind(updatedLikes, new Date().toISOString(), id).run();
    ...
    if (path.startsWith("/api/comments/") && request.method === "DELETE") {
      const id = path.replace("/api/comments/", "");
