        ...
        <div class="container mx-auto px-4 flex justify-between items-center">
          <div class="max-w-1/3">
            ...
            <div class="flex space-x-3">
              <router-link :to="`/detail/${video.id}`" class="bg-red-600 text-white px-5 py-2 rounded hover:bg-red-700 flex items-center text-sm">
          ...
          <div class="bg-black/80 rounded-lg p-3 w-64">
            ...
            <ul class="space-y-2">
              <li v-for="(item, idx) in hotRanking" :key="idx" class="flex items-center">
                ...
                <router-link :to="`/detail/${item.id}`" class="ml-2 text-white text-sm hover:text-red-600 line-clamp-1">{{ item.title }}</router-link>
              </li>
            </ul>
          </div>
        </div>
